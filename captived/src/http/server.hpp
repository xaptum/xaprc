...
public:
  ...
  template <typename Resource> int register_resource(Resource& rest_resource);
...
inline void server::respond_not_allowed(struct evhttp_request* req,
                                        std::string err) {
  evhttp_send_error(req, HTTP_BADMETHOD, err.c_str());
}

inline void server::respond_bad_request(struct evhttp_request* req,
                                        std::string err) {
