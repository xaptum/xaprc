...
public:
  resource(std::string path) : path_(path) {}

  virtual ~resource() = default;

  std::string get_path() { return path_; }

  virtual resp_type get(req_type body) { return not_allowed("GET"); }

  virtual resp_type put(req_type body) { return not_allowed("PUT"); }

  virtual resp_type post(req_type body) { return not_allowed("POST"); }

protected:
  virtual resp_type ok(resp_body_type body) { return {http::status::ok, body}; }
