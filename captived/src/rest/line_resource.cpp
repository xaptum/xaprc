...
std::experimental::optional<std::string> line_resource::line() {
  return system_.readline(filename_);
}

bool line_resource::line(std::string new_line) {
  if (!writable_)
    return false;
...
line_resource::resp_type line_resource::get(resource::req_type) {
  auto line_opt = line();
  if (!line_opt) {
...
line_resource::resp_type line_resource::put(resource::req_type body) {
  if (!writable_)
