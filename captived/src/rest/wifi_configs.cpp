#include "json.hpp"
#include "rest/wifi_configs.hpp"

namespace captived {
namespace rest {

namespace {

  /**
   * Adds the given resource as child element of the provided JSON
   * object under the specified key.
   */
  void add_resource(json::json_type& obj, std::string key, resource& res) {
    resource::resp_type resp;
    json::json_type val;

    resp = res.get(json::null());
    if (std::get<0>(resp) == http::status::ok)
      val = std::get<1>(resp);
    else
      val = json::null();

    json::object_set(obj, key, val);
  }

}

  wifi_configs::wifi_configs(std::string path,
                             wifi_config& passthrough,
                             wifi_config& secure_host) :
    resource(path),
    passthrough_(passthrough),
    secure_host_(secure_host)
  {}

  wifi_configs::resp_type
  wifi_configs::get(req_type) {
    auto obj = json::object();

    add_resource(obj, "passthrough", passthrough_);
    add_resource(obj, "secure_host", secure_host_);

    return ok(obj);
}

} // namespace rest
} // namespace captived
