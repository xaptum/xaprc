...
std::experimental::optional<std::string> wifi_config::contents() {
  return system_.read(config_file_);
}

bool wifi_config::contents(std::string new_contents) {
  return system_.write(config_file_, new_contents);
}

std::experimental::optional<std::string> wifi_config::sha256() {
...
wifi_config::resp_type wifi_config::get(req_type) {
  auto contents_str = contents();
  if (!contents_str) {
...
resource::resp_type wifi_config::put(resource::req_type body) {
