...
namespace json {
  ...
  static
  json_type json(json_t* ref)
  {
    return {ref, &json_decref};
  }

  static
  json_type boolean(bool val)
  {
  ...
  static
  json_type string(const std::string str)
  {
    return json(json_string(str.c_str()));
  }

  static
  json_type string(const std::stringstream& stream)
  {
    return string(stream.str());
  }

  static
  json_type number(json_int_t val)
  {
    return json(json_integer(val));
  }

  static
  json_type number(double val)
  {
  ...
  static
  int object_set(json_type obj, std::string key, json_type val)
  {
    return json_object_set(obj.get(), key.c_str(), val.get());
  }

  static
  std::string dumps(json_type json)
  {
  ...
  static
  json_type loads(std::string str, error_type* error)
  {
